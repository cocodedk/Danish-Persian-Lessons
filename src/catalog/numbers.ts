 ...
 * 100–900 rule lesson's candidate rows. The order is append-only so existing
 * ids and generated review-row order stay put, and each lesson contributes
 * only its own rows — rows it merely references stay with their own module, so
 * no entry is registered twice.
 */
