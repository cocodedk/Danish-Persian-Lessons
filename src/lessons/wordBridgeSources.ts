/** Review sources stay out of the learner bundle but share the bridge IDs. */
export const wordBridgeSources: Record<string, readonly string[]> = {
  'dandan-tand': [
    'https://lrc.la.utexas.edu/lex/master/0441',
    'https://parsianjoman.org/wp-content/uploads/2022/12/An-Etymological-Dictionary-of-the-Persian-Language.pdf',
    'https://ordnet.dk/ddo/ordbog/tand',
  ],
  'setad-sted': [
    'https://vajehyab.com/dehkhoda/%D8%B3%D8%AA%D8%A7%D8%AF-2',
    'https://ordnet.dk/ddo/ordbog/sted',
  ],
  'band-baand': [
    'https://www.iranicaonline.org/articles/band-dam/',
    'https://ordnet.dk/ddo/ordbog/b%C3%A5nd',
    'https://ordnet.dk/ddo/ordbog/vand',
  ],
  'seyl-sejle': [
    'https://www.iranicaonline.org/articles/floods/',
    'https://www.arabicacademy.gov.eg/ar/%D9%85%D8%AD%D8%B1%D9%83-%D8%A7%D9%84%D8%A8%D8%AD%D8%AB/%D9%85%D8%B9%D8%AC%D9%85/%D8%A7%D9%84%D9%85%D8%B9%D8%AC%D9%85-%D8%A7%D9%84%D9%88%D8%B3%D9%8A%D8%B7-19/%D8%B3%D9%8A%D9%84?exact_search=true',
    'https://ordnet.dk/ddo/ordbog/sejle',
  ],
}
